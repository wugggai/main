...
def handle_message_create_request(db: Session, user_id: int, interaction: InteractionModel, message_create_params: MessageCreate):
  """
  ...
  if not requested_model:
    raise HTTPException(status_code=404, detail=f'{message_create_params.model} is not availble')
  # verifies that user has provided api_key
  api_key = requested_model.get_user_api_key(db=db, user_id=user_id)
  if api_key is None:
    raise HTTPException(status_code=403, detail=f'No API key provided for {message_create_params.model}')
  
  interaction_context = requested_model.get_interaction_context(db=db, interaction=interaction)
  try:
    model_res_msg = requested_model.post_message(api_key=api_key, interaction_context=interaction_context, message_create_params=message_create_params)
  except Exception as e:
    raise HTTPException(status_code=503, detail=f'Cannot send message to {message_create_params.model}: {e}')

  create_message(db=db, interaction=interaction, source=f'user_{user_id}', message=message_create_params.message)
  ai_message = create_message(db=db, interaction=interaction, source=message_create_params.model, message=model_res_msg)
