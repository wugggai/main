...
def pre_authorization_wrapper_helper(db_func: Callable, get_owner_ids_from_kwargs_func: Callable):
  @wraps(db_func)
...
def authorize_by_res_owner_id(db_func: Callable):
  ...
  return pre_authorization_wrapper_helper(db_func, lambda kwargs: get_owner_ids_from_params(kwargs))

# authorization_post_check_helper:
# A "post-authorization" checks an object's property after it's fetched from db
def post_authorization_wrapper_helper(db_func: Callable, get_owner_ids_from_fetched_object: Callable):
  @wraps(db_func)
  def wrapper(*args, **kwargs):
...
def authorize_get_tag_from_db(db_func: Callable):
  return post_authorization_wrapper_helper(db_func, lambda tag: [tag.creator_user_id] if tag else None)

def authorize_get_tags_from_db(db_func: Callable):
  return post_authorization_wrapper_helper(db_func, lambda tags: list(set([tag.creator_user_id for tag in tags])) if tags else [])

def authorize_get_interaction_from_db(db_func: Callable):
  return post_authorization_wrapper_helper(db_func, lambda interaction: [interaction.creator_user_id] if interaction else None)
