...
from wugserver.models.db.user_model import UserModel
...
def create_message_route(interaction_id: UUID, message_create_params: MessageCreate, db: Session = Depends(get_db), current_user: UserModel = Depends(get_current_active_user)):
  ...
  return handle_message_create_request(db=db, user_id=current_user.id, interaction=interaction, message_create_params=message_create_params)
