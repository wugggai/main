...
def create_message(interaction_id: UUID, message_create_params: MessageCreate, db: Session = Depends(get_db)):
...
def get_messages(interaction_id: UUID, offset: int = 0, limit: int = 15, from_latest: bool = True, db: Session = Depends(get_db)):
